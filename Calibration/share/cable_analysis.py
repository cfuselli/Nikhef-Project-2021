...
def make_hist(id,data,xname,bins=256,x_range=(0,1023),label=None,alpha=0.35):
    plt.figure(id)
    data, bin_x , _ = plt.hist(data,bins=bins,range=x_range,density=True,alpha=alpha,label=label)
    plt.xlabel(xname)
    plt.ylabel('Entries')
...
make_hist(4,timediff_cable,'Time diff [s]',label='total %s'%DISTINCTION,x_range=(0,60),bins=60)
make_hist(4,timediff_scint,'Time diff [s]',label='total %s'%dist2_label,x_range=(0,60),bins=60)


if DISTINCTION_2 is not None:
    make_hist(3,adc_else,'ADC',label='total rest',bins=128)
    make_hist(4,timediff_scint,'Time diff [s]',label='total rest',x_range=(0,60),bins=60)
