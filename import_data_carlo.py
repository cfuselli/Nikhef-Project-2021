...
if mode == 1:
    ...
    if nDetectors>1:
        for i in range(nDetectors):
            string_of_names += detector_name_list[i] +b', '
    else:
        string_of_names+=detector_name_list[0]

    #print(string_of_names)
    # file.write(b'Device ID(s): '+ string_of_names)
    file.write(b'\n')
    ...
    while True:
        for i in range(nDetectors):
            if globals()['Det%s' % str(i)].inWaiting():
                ...
                file.write(str.encode(str(datetime.now())+" "+str(data)+" "+str(detector_name_list[i])+'\n'))
                print(data)
                print(str.encode(str(datetime.now())+" "+str(data)+" "+str(detector_name_list[i])+'\n'))
                plt.scatter(datetime.now(), 100, s=20)
                plt.pause(0.0005)
...
# Working on it.. does not read SDcard
if mode == 2:
    ...
    if ComPort.readline().strip() == 'CosmicWatchDetector':
        ...
        while True:
            ...
            if b'Done' in data:
                ComPort.close()
                sys.exit()
            elif b'opening:' in data:
                fname = dir_path + '/' + str(data.split(b' ')[-1].split(b'.txt')[0])+'.txt'
...
if mode == 4:
    ...
    #bg.daemon = True
    #bg.start()
    #thread.start_new_thread(RUN,(bg,))  ## CARLO REMOVED IT
